...
N = 21;      % Number of control intervals per phase
...
tp1_0 = 460;   % Initial final time phase 1 (min)
...
T = [casadi.MX(0); casadi.MX.sym('T'); casadi.MX(Tf)];
...
t_sol = [ linspace(0,t_p, nGrid); ...
    linspace(t_p,1440, nGrid) ];

x_sol = [ w_sol(1:nGrid)'; ...
    w_sol(nGrid+1:2*nGrid)'];

u_sol = [ w_sol(2*nGrid+1:3*nGrid)'; ...
    w_sol(3*nGrid+1:4*nGrid)'];
...
nInter = 10*nGrid;
t_inter = [ linspace(0, t_p, nInter); ...
    linspace(t_p, 1440, nInter) ];
...
f_h2_prd_opt =  zeros(2, nInter);
v_el_opt = zeros(2, nInter);
p_el_opt = zeros(2, nInter);
p_ps_opt = zeros(2, nInter);
p_grid = zeros(2, nInter);
p_grid2 = zeros(2, nInter);
for p=1:2
    for i=1:nInter
